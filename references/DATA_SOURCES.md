...
已验证可访问性（2026-08-12 追加）：
- 统计局 70 城公告页表3/表4（面积段分类指数）结构确认：表头行（城市 + 三面积段）+ 子表头行（环比/同比/1-N月平均 ×3）+ 基准行（=100）+ 数据行；每页渲染两遍需去重；已脚本化进 fetch_data.py。
- 央行 LPR 列表页分页模式：`{栏目目录}/de24575c-{页码}.html`，页码与总页数在页面 `tagname` / `totalpage` 属性中；href 为站点根路径；已脚本化进 scripts/fetch_lpr.py（全序列实测约 84 期/5 页）。
- 统计局 RSS 实测覆盖期次：2024-01 至 2026-06 共 30 期（滚动窗口，会持续前移；2023-09 起的历史期次已滚出）。

已验证可访问性（2026-08-13 追加）：
- 统计局归档页 https://www.stats.gov.cn/sj/zxfb/ ：分页 index.html/index_N.html 共 67 页（约 15 条/页），
  最早公告约 2021-09-15（对应期次 2021-08）；已脚本化进 scripts/fetch_archive.py
  （回溯得 2021-08 至 2023-12 共 29 期）。**2020-01 至 2021-07 超出归档覆盖**，
  需 data.stats.gov.cn 在线查询或人工补采。
- 央行房贷加权平均利率栏目（5470606）：静态列表 8 期（2024-Q3 起，栏目新建较晚），
  详情页正则「个人住房贷款加权平均利率为X.XX%」与 meta PubDate 公告日期均可解析；已脚本化进 scripts/fetch_pbc.py。
- 央行信贷收支/M2/社融旧路径（diaochatongjisi/116636/116756、116762）已 404，
  现站点对应栏目为 JS 动态加载，无静态列表——按 SKILL.md 兜底规则用 WebSearch 处理。
- 住建部：政策栏目库页（zhengce/zhengcefilelib、gongkai/zc/*）为 JS 动态加载不可静态抓；
  首页与文章详情页（art_*.html）为静态，可提取标题/链接/发布时间；
  已脚本化进 scripts/fetch_policies.py（首页快照 + 详情页日期）。
